      ...
      <AnimatePresence>
        {hearts.map((heart) => (
          <motion.div
            ...
            animate={{ scale: [0, 1.5, 1], opacity: [1, 1, 0], y: -100, x: (Math.random() - 0.5) * 100 }}
            exit={{ opacity: 0 }}
            transition={{ duration: 1.5, ease: "easeOut" }}
            onAnimationComplete={() => { setHearts(prev => prev.filter(h => h.id !== heart.id)); }}
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="url(#heartGradientGold)" className="drop-shadow-lg">
              <defs>
                <linearGradient id="heartGradientGold" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="hsl(38, 65%, 60%)" />
